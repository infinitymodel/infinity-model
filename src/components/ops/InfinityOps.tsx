...
import { hasPermission, type OpsRole } from "@/lib/ops/permissions";
import { operationalPricingDefaults, type PricingResult } from "@/lib/ops/pricing";
...
function canMoveOrder(role: OpsRole, current: OpsStage, next: OpsStage) {
  if (role === "owner" || role === "admin") return true;
  if (role === "sales") return ["brief", "design", "quote", "production"].includes(current) && ["brief", "design", "quote", "production"].includes(next);
  if (role === "production") return ["production", "quality"].includes(current) && ["production", "quality"].includes(next);
  return false;
}

function buildCustomers(orders: OpsOrder[]): OpsCustomer[] {
  const customers = new Map<string, OpsCustomer>();
  for (const order of orders) {
    const key = `${order.customer}|${order.contact}`;
    ...
    customers.set(key, {
      id: key,
      name: order.customer,
      company: order.company || "عميل فردي",
      phone: order.contact || "غير مسجل",
      totalOrders: 1,
      totalValue: order.value,
      lastOrder: order.createdAt,
    });
  }
  return Array.from(customers.values()).sort((a, b) => b.lastOrder.localeCompare(a.lastOrder));
}
...
export default function InfinityOps({ operator, initialOrders, initialInventory }: { operator: { name: string; role: OpsRole }; initialOrders: OpsOrder[]; initialInventory: InventoryItem[] }) {
  ...
  const canCreateOrder = hasPermission(operator.role, "orders:write");
  const canAdjustInventory = hasPermission(operator.role, "inventory:write");
  const canCalculatePrice = hasPermission(operator.role, "pricing:calculate");

  async function changeStage(order: OpsOrder, stage: OpsStage) {
    if (!canMoveOrder(operator.role, order.stage, stage)) {
      setNotice("هذه المرحلة خارج نطاق صلاحية حسابك.");
      return;
    }
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f4f5f2] text-[#171717]">
        ...
        <main className="min-w-0 flex-1 px-4 py-4 sm:px-6 lg:px-10 lg:py-7">
          <header className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-200 pb-5">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button disabled={!canCreateOrder} onClick={() => setShowNewOrder(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#171717] px-4 py-3 text-sm font-bold text-white shadow-lg shadow-zinc-900/10 transition hover:bg-[#c59b27] hover:text-[#171717] disabled:cursor-not-allowed disabled:opacity-45"><Plus className="h-4 w-4" />طلب جديد</button>
            </div>
          </header>

          {view === "dashboard" && <Dashboard orders={orders} activeOrders={activeOrders} activeValue={activeValue} productionOrders={productionOrders} lowStock={lowStock} customers={customers} onView={setView} onStageChange={changeStage} role={operator.role} today={today} />}
          {view === "orders" && <OrdersView orders={filteredOrders} onStageChange={changeStage} onCreate={() => setShowNewOrder(true)} role={operator.role} canCreate={canCreateOrder} />}
          {view === "production" && <ProductionView orders={orders} onStageChange={changeStage} role={operator.role} />}
          {view === "inventory" && <InventoryView inventory={inventory} onAdjust={updateInventory} canAdjust={canAdjustInventory} />}
          {view === "customers" && <CustomersView orders={orders} customers={customers} />}
          {view === "pricing" && <PricingView allowed={canCalculatePrice} />}
        </main>
      </div>
      ...
      {showNewOrder && canCreateOrder && <NewOrderModal onClose={() => setShowNewOrder(false)} onSubmit={addOrder} />}
    </div>
  );
}

function Dashboard({ orders, activeOrders, activeValue, productionOrders, lowStock, customers, onView, onStageChange, role, today }: { orders: OpsOrder[]; activeOrders: OpsOrder[]; activeValue: number; productionOrders: OpsOrder[]; lowStock: InventoryItem[]; customers: OpsCustomer[]; onView: (view: View) => void; onStageChange: (order: OpsOrder, stage: OpsStage) => void; role: OpsRole; today: number }) {
  ...
  return <div className="py-7">
    ...
    <section className="mt-7 grid gap-6 lg:grid-cols-2"><div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-sm font-black">تنبيهات المخزون</p><p className="mt-1 text-xs text-zinc-500">أصناف تتطلب قرار شراء</p></div><button onClick={() => onView("inventory")} className="text-xs font-bold text-[#a7801a]">عرض المخزون</button></div><div className="mt-5 space-y-3">{lowStock.length ? lowStock.map((item) => <div key={item.id} className="flex items-center justify-between rounded-2xl bg-red-50 px-4 py-3"><div><p className="text-sm font-bold">{item.name} <span className="font-medium text-zinc-500">— {item.brand}</span></p><p className="mt-1 text-xs text-red-700">{item.location}</p></div><span className="rounded-lg bg-white px-3 py-1.5 text-sm font-black text-red-700">{item.available} {item.unit}</span></div>) : <p className="rounded-2xl bg-emerald-50 px-4 py-5 text-sm font-bold text-emerald-700">لا توجد تنبيهات مخزون حاليًا.</p>}</div></div><div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-sm font-black">عملاء نشطون</p><p className="mt-1 text-xs text-zinc-500">مُستخرجون من طلباتك الفعلية</p></div><button onClick={() => onView("customers")} className="text-xs font-bold text-[#a7801a]">ملف العملاء</button></div><div className="mt-5 space-y-3">{customers.length ? customers.slice(0, 3).map((customer) => <div key={customer.id} className="flex items-center justify-between rounded-2xl bg-zinc-50 px-4 py-3"><div><p className="text-sm font-bold">{customer.name}</p><p className="mt-1 text-xs text-zinc-500">{customer.totalOrders} طلبات • آخر طلب {formatDate(customer.lastOrder)}</p></div><p className="text-sm font-black text-[#a7801a]">{currency(customer.totalValue)}</p></div>) : <p className="rounded-2xl bg-zinc-50 px-4 py-5 text-sm font-bold text-zinc-500">سيظهر العملاء هنا بعد تسجيل أول طلب.</p>}</div></div></section>
  </div>;
}

function OrdersView({ orders, onStageChange, onCreate, role, canCreate }: { orders: OpsOrder[]; onStageChange: (order: OpsOrder, stage: OpsStage) => void; onCreate: () => void; role: OpsRole; canCreate: boolean }) {
  return <div className="py-7"><section className="rounded-3xl border border-zinc-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-100 p-5 sm:p-6"><div><p className="text-lg font-black">سجل الطلبات</p><p className="mt-1 text-sm text-zinc-500">تابع حالة الطلب من الاستلام حتى التسليم.</p></div><button disabled={!canCreate} onClick={onCreate} className="inline-flex items-center gap-2 rounded-xl bg-[#171717] px-4 py-3 text-sm font-bold text-white hover:bg-[#c59b27] hover:text-[#171717] disabled:cursor-not-allowed disabled:opacity-40"><FilePlus2 className="h-4 w-4" />إضافة طلب مخصص</button></div><div className="overflow-x-auto"><table className="w-full min-w-[950px] text-right"><thead className="bg-zinc-50 text-xs text-zinc-500"><tr><th className="px-6 py-4 font-bold">رقم الطلب</th><th className="px-6 py-4 font-bold">العميل والمشروع</th><th className="px-6 py-4 font-bold">الخامة / الكمية</th><th className="px-6 py-4 font-bold">القيمة</th><th className="px-6 py-4 font-bold">الحالة</th><th className="px-6 py-4 font-bold">الموعد</th><th className="px-6 py-4 font-bold">تحريك</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id} className="border-t border-zinc-100 transition hover:bg-[#c59b27]/[0.035]"><td className="px-6 py-5 font-black text-[#a7801a]" dir="ltr">{order.id}</td><td className="px-6 py-5"><p className="font-bold">{order.title}</p><p className="mt-1 text-xs text-zinc-500">{order.customer} • {order.service}</p></td><td className="px-6 py-5 text-sm font-semibold text-zinc-600">{order.material}<span className="mr-2 text-zinc-400">×</span>{order.quantity}</td><td className="px-6 py-5 text-sm font-black">{currency(order.value)}</td><td className="px-6 py-5"><div className="flex items-center gap-2"><StageBadge stage={order.stage} /><PriorityBadge priority={order.priority} /></div></td><td className="px-6 py-5 text-sm font-semibold text-zinc-600">{formatDate(order.dueDate)}</td><td className="px-6 py-5"><div className="flex gap-1"><button disabled={order.stage === "brief" || !canMoveOrder(role, order.stage, stagePrevious(order.stage))} onClick={() => onStageChange(order, stagePrevious(order.stage))} className="rounded-lg border border-zinc-200 p-2 text-zinc-600 disabled:opacity-30" aria-label="المرحلة السابقة"><ArrowLeft className="h-4 w-4" /></button><button disabled={order.stage === "delivery" || !canMoveOrder(role, order.stage, stageNext(order.stage))} onClick={() => onStageChange(order, stageNext(order.stage))} className="rounded-lg border border-zinc-200 p-2 text-zinc-600 hover:border-[#c59b27] hover:text-[#a7801a] disabled:opacity-30" aria-label="المرحلة التالية"><ChevronLeft className="h-4 w-4" /></button></div></td></tr>)}</tbody></table>{orders.length === 0 && <p className="p-10 text-center text-sm font-semibold text-zinc-500">لا توجد نتائج مطابقة.</p>}</div></section></div>;
}

function ProductionView({ orders, onStageChange, role }: { orders: OpsOrder[]; onStageChange: (order: OpsOrder, stage: OpsStage) => void; role: OpsRole }) {
  const stages: OpsStage[] = ["brief", "design", "quote", "production", "quality", "delivery"];
  return <div className="py-7"><div className="mb-5"><p className="text-lg font-black">لوحة الإنتاج</p><p className="mt-1 text-sm text-zinc-500">حرّك كل طلب عبر المراحل باستخدام الأسهم داخل البطاقة.</p></div><div className="grid gap-4 2xl:grid-cols-6 xl:grid-cols-3 md:grid-cols-2">{stages.map((stage) => { const items = orders.filter((order) => order.stage === stage); return <section key={stage} className="min-h-64 rounded-3xl border border-zinc-200 bg-white/70 p-3"><div className="mb-3 flex items-center justify-between px-2 pt-1"><p className="text-sm font-black">{stageMeta[stage].shortLabel}</p><span className="rounded-lg bg-zinc-100 px-2 py-1 text-xs font-bold text-zinc-600">{items.length}</span></div><div className="space-y-3">{items.map((order) => <article key={order.id} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-black leading-5">{order.title}</p><p className="mt-1 text-xs font-bold text-[#a7801a]" dir="ltr">{order.id}</p></div><PriorityBadge priority={order.priority} /></div><div className="mt-4 space-y-2 text-xs font-semibold text-zinc-500"><p>{order.customer}</p><p>{order.material} × {order.quantity}</p><p>موعد: {formatDate(order.dueDate)}</p></div><div className="mt-4 flex gap-2"><button disabled={stage === "brief" || !canMoveOrder(role, stage, stagePrevious(stage))} onClick={() => onStageChange(order, stagePrevious(stage))} className="flex-1 rounded-lg border border-zinc-200 px-2 py-2 text-xs font-bold text-zinc-600 disabled:opacity-30">سابق</button><button disabled={stage === "delivery" || !canMoveOrder(role, stage, stageNext(stage))} onClick={() => onStageChange(order, stageNext(stage))} className="flex-1 rounded-lg bg-[#171717] px-2 py-2 text-xs font-bold text-white transition hover:bg-[#c59b27] hover:text-[#171717] disabled:opacity-30">التالي</button></div></article>)}</div></section>; })}</div></div>;
}

function InventoryView({ inventory, onAdjust, canAdjust }: { inventory: InventoryItem[]; onAdjust: (id: string, change: number) => void; canAdjust: boolean }) {
  const typeLabel = { filament: "فيلمنت", resin: "ريزن", spare: "قطع غيار" };
  return <div className="py-7"><div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-lg font-black">مخزون الورشة</p><p className="mt-1 text-sm text-zinc-500">تعديلات الكمية تحفظ مباشرة في قاعدة البيانات مع تطبيق الصلاحيات.</p></div><div className="rounded-xl bg-[#c59b27]/15 px-4 py-3 text-sm font-bold text-[#765600]">{inventory.filter((item) => item.available <= item.reorderPoint).length} أصناف تحتاج إعادة طلب</div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{inventory.map((item) => { const isLow = item.available <= item.reorderPoint; return <article key={item.id} className={`rounded-3xl border bg-white p-5 shadow-sm ${isLow ? "border-red-200" : "border-zinc-200"}`}><div className="flex items-start justify-between"><div><span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-bold text-zinc-600">{typeLabel[item.type]}</span><p className="mt-4 text-lg font-black">{item.name}</p><p className="mt-1 text-sm font-semibold text-zinc-500">{item.brand} • {item.color}</p></div><button className="rounded-xl p-2 text-zinc-400 hover:bg-zinc-100" aria-label="خيارات المخزون"><MoreHorizontal className="h-5 w-5" /></button></div><div className="mt-6 flex items-end justify-between"><div><p className="text-xs font-bold text-zinc-500">المتاح الآن</p><p className={`mt-1 text-3xl font-black ${isLow ? "text-red-600" : "text-zinc-900"}`}>{item.available}<span className="mr-1 text-sm">{item.unit}</span></p></div><p className="text-xs font-semibold text-zinc-500">حد الطلب: {item.reorderPoint}</p></div><div className="mt-5 flex items-center justify-between border-t border-zinc-100 pt-4"><p className="text-xs font-semibold text-zinc-500">{item.location}</p><div className="flex gap-2"><button disabled={!canAdjust} onClick={() => onAdjust(item.id, -1)} className="h-9 w-9 rounded-lg border border-zinc-200 text-lg font-black text-zinc-600 hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-30">−</button><button disabled={!canAdjust} onClick={() => onAdjust(item.id, 1)} className="h-9 w-9 rounded-lg bg-[#171717] text-lg font-black text-white hover:bg-[#c59b27] hover:text-[#171717] disabled:cursor-not-allowed disabled:opacity-30">+</button></div></div></article>; })}</div>{!inventory.length && <p className="mt-5 rounded-2xl bg-zinc-100 p-6 text-center text-sm font-bold text-zinc-500">أضف أصناف المخزون من قاعدة البيانات لتظهر هنا.</p>}</div>;
}

function CustomersView({ orders, customers }: { orders: OpsOrder[]; customers: OpsCustomer[] }) {
  return <div className="py-7"><section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-lg font-black">علاقات العملاء</p><p className="mt-1 text-sm text-zinc-500">نظرة موحدة مبنية على العملاء والطلبات المسجلة فعليًا.</p></div><div className="rounded-xl bg-zinc-100 px-4 py-2 text-sm font-bold text-zinc-600">{customers.length} عملاء نشطون</div></div><div className="mt-6 grid gap-4 lg:grid-cols-2">{customers.map((customer) => { const openOrders = orders.filter((order) => order.customer === customer.name && order.stage !== "delivery"); return <article key={customer.id} className="rounded-2xl border border-zinc-200 p-5 transition hover:border-[#c59b27]/50 hover:shadow-md"><div className="flex items-start justify-between gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#171717] text-sm font-black text-[#e3bd50]">{customer.name.slice(0, 1)}</div><button className="rounded-lg p-2 text-zinc-400 hover:bg-zinc-100" aria-label="خيارات العميل"><MoreHorizontal className="h-5 w-5" /></button></div><p className="mt-4 text-lg font-black">{customer.name}</p><p className="mt-1 text-sm text-zinc-500">{customer.company}</p><div className="mt-5 grid grid-cols-3 gap-3 border-t border-zinc-100 pt-4"><Stat label="إجمالي الطلبات" value={customer.totalOrders.toString()} /><Stat label="قيد التنفيذ" value={openOrders.length.toString()} /><Stat label="إجمالي القيمة" value={compactNumber(customer.totalValue)} /></div><div className="mt-5 flex items-center justify-between rounded-xl bg-zinc-50 px-3 py-2.5 text-xs font-semibold text-zinc-600"><span dir="ltr">{customer.phone}</span><span>آخر طلب {formatDate(customer.lastOrder)}</span></div></article>; })}</div>{!customers.length && <p className="mt-6 rounded-2xl bg-zinc-50 p-8 text-center text-sm font-bold text-zinc-500">لا توجد علاقات عملاء بعد. أضف طلبًا جديدًا لبدء بناء سجل العميل.</p>}</section></div>;
}

function PricingView({ allowed }: { allowed: boolean }) {
  ...
  async function calculate(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/ops/pricing/calculate", {
      ...
      body: JSON.stringify({
        ...
        powerRatePerHour: Number(data.get("powerRatePerHour")),
        overheadPercent: Number(data.get("overheadPercent")),
        shippingCost: Number(data.get("shippingCost")),
        discountPercent: Number(data.get("discountPercent")),
        includeVat: data.get("includeVat") === "on",
      }),
    });
  ...
  return <div className="py-7"><div className="mb-6"><p className="text-lg font-black">التسعير التلقائي</p><p className="mt-1 text-sm text-zinc-500">تسعير قابل للمراجعة يشمل الخامة والتشغيل والعمالة والكهرباء والهدر والمصاريف العامة والربح والشحن والضريبة.</p></div><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]"><section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-7"><form onSubmit={calculate} className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-bold text-zinc-700">الخامة<select name="materialCode" defaultValue="pla" className="rounded-xl border border-zinc-200 bg-white px-3 py-3 outline-none focus:border-[#c59b27]"><option value="pla">PLA</option><option value="petg">PETG</option><option value="asa">ASA</option><option value="abs">ABS</option><option value="resin">ريزن</option></select></label><label className="grid gap-2 text-sm font-bold text-zinc-700">درجة التعقيد<select name="complexity" defaultValue="standard" className="rounded-xl border border-zinc-200 bg-white px-3 py-3 outline-none focus:border-[#c59b27]"><option value="basic">بسيط</option><option value="standard">قياسي</option><option value="complex">معقد</option></select></label><Input label="وزن القطعة (غرام)" name="weightGrams" type="number" min="1" defaultValue="100" required /><Input label="وقت الطباعة للقطعة (ساعة)" name="printHours" type="number" min="0" step="0.1" defaultValue="3" required /><Input label="العمل اليدوي للقطعة (دقيقة)" name="labourMinutes" type="number" min="0" defaultValue="20" required /><Input label="الكمية" name="quantity" type="number" min="1" defaultValue="1" required /><Input label="الكهرباء لكل ساعة (ر.س)" name="powerRatePerHour" type="number" min="0" step="0.1" defaultValue={operationalPricingDefaults.powerRatePerHour} required /><Input label="مصاريف التشغيل (%)" name="overheadPercent" type="number" min="0" max="100" step="0.5" defaultValue={operationalPricingDefaults.overheadPercent} required /><Input label="الشحن (ر.س)" name="shippingCost" type="number" min="0" step="0.5" defaultValue="0" required /><Input label="خصم تجاري (%)" name="discountPercent" type="number" min="0" max="80" step="0.5" defaultValue="0" required /><label className="sm:col-span-2 flex items-center gap-3 rounded-xl bg-zinc-50 px-4 py-3 text-sm font-bold text-zinc-700"><input name="includeVat" type="checkbox" defaultChecked className="h-4 w-4 accent-[#c59b27]" />إضافة ضريبة القيمة المضافة 15%</label><div className="sm:col-span-2 mt-2 border-t border-zinc-100 pt-5"><button disabled={loading || !allowed} className="inline-flex items-center gap-2 rounded-xl bg-[#171717] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#c59b27] hover:text-[#171717] disabled:cursor-not-allowed disabled:opacity-60"><Calculator className="h-4 w-4" />{loading ? "جارٍ الحساب…" : "احسب السعر المقترح"}</button>{!allowed && <p className="mt-3 text-sm font-bold text-red-700">لا تملك صلاحية استخدام حاسبة التسعير.</p>}</div></form>{error && <p role="alert" className="mt-5 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p>}</section><PricingResultCard result={result} /></div><p className="mt-5 text-xs leading-6 text-zinc-500">تعتمد الخامة والآلة والعمالة والتجهيز والهدر ونسبة الربح على قاعدة التسعير. تُسجّل الإضافات التجارية في هذه الحسبة فقط ويجب اعتماد السعر قبل مشاركته مع العميل.</p></div>;
}
...
function PricingResultCard({ result }: { result: PricingResult | null }) {
  ...
  const rows = [["الخامة", result.materialCost], ["وقت الطباعة", result.machineCost], ["العمل اليدوي", result.labourCost], ["الكهرباء", result.powerCost], ["التجهيز", result.setupFee], ["الهدر", result.wasteCost], ["التعقيد", result.complexityCost], ["المصاريف العامة", result.overheadCost], ["الربح", result.markup], ["الخصم", -result.discount], ["الشحن", result.shippingCost], ["الضريبة", result.vat]] as const;
  return <aside className="rounded-3xl bg-[#171717] p-6 text-white shadow-xl"><p className="text-xs font-bold text-[#e3bd50]">سعر مقترح للعميل</p><p className="mt-2 text-4xl font-black">{currency(result.total)}</p><p className="mt-2 text-sm text-zinc-400">سعر الوحدة: {currency(result.unitPrice)} • قبل الضريبة: {currency(result.netBeforeVat)}</p><div className="mt-6 space-y-3 border-t border-white/10 pt-5">{rows.map(([label, value]) => <div className="flex items-center justify-between text-sm" key={label}><span className="text-zinc-400">{label}</span><span className={`font-bold ${value < 0 ? "text-emerald-300" : ""}`}>{currency(value)}</span></div>)}</div></aside>;
}

function NewOrderModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div role="dialog" aria-modal="true" aria-labelledby="new-order-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" dir="rtl"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-[#a7801a]">Infinity OS</p><h2 id="new-order-title" className="mt-1 text-2xl font-black">إضافة طلب مخصص</h2><p className="mt-2 text-sm text-zinc-500">يبدأ الطلب في مرحلة الاستلام ثم ينتقل عبر لوحة الإنتاج.</p></div><button onClick={onClose} className="rounded-xl border border-zinc-200 p-2 text-zinc-600 hover:bg-zinc-100" aria-label="إغلاق"><X className="h-5 w-5" /></button></div><form onSubmit={onSubmit} className="mt-7 grid gap-4 sm:grid-cols-2"><Input label="اسم العميل" name="customer" required placeholder="الاسم أو اسم المنشأة" /><Input label="اسم الشركة (اختياري)" name="company" placeholder="اسم المنشأة" /><Input label="اسم المشروع" name="title" required placeholder="مثال: حامل لوحة تحكم" /><label className="grid gap-2 text-sm font-bold text-zinc-700">الخدمة<select name="service" required defaultValue="" className="rounded-xl border border-zinc-200 bg-white px-3 py-3 outline-none focus:border-[#c59b27]"><option value="" disabled>اختر الخدمة</option><option>طباعة ثلاثية الأبعاد</option><option>تصميم CAD ثلاثي الأبعاد</option><option>نموذج أولي</option><option>طباعة ريزن</option><option>هندسة عكسية</option><option>إنتاج دفعات وهدايا</option><option>صيانة طابعات ثلاثية الأبعاد</option></select></label><Input label="الخامة" name="material" placeholder="PLA، PETG، ريزن…" /><Input label="رقم التواصل" name="contact" type="tel" placeholder="+966 5X XXX XXXX" /><Input label="الكمية" name="quantity" type="number" defaultValue="1" min="1" required /><Input label="قيمة تقديرية (ر.س)" name="value" type="number" min="0" defaultValue="0" required /><Input label="موعد التسليم" name="dueDate" type="date" required /><label className="grid gap-2 text-sm font-bold text-zinc-700">الأولوية<select name="priority" className="rounded-xl border border-zinc-200 bg-white px-3 py-3 outline-none focus:border-[#c59b27]"><option value="medium">متوسط</option><option value="high">عاجل</option><option value="low">منخفض</option></select></label><div className="sm:col-span-2 mt-2 flex justify-end gap-3 border-t border-zinc-100 pt-5"><button type="button" onClick={onClose} className="rounded-xl border border-zinc-200 px-5 py-3 text-sm font-bold text-zinc-700 hover:bg-zinc-50">إلغاء</button><button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-[#171717] px-5 py-3 text-sm font-bold text-white hover:bg-[#c59b27] hover:text-[#171717]"><Plus className="h-4 w-4" />إضافة الطلب</button></div></form></div></div>;
}
